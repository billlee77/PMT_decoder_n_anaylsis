...
void goodRICHEvent::Timewalk_Calabration(TTree* t1) {
	...
	t1->Draw("delta_t:adc", "adc < 1300 && adc > 400 && delta_t > 10 && delta_t < 70", "colz");
	...
	c10->cd(2);
	t1->Draw("delta_t:adc >> delta_adc(900, 400, 1300, 60, 10, 70)", "adc < 1300 && adc > 400 && delta_t > 10 && delta_t < 70 && mycut", "colz");
	TH1 *dt_adc_cut = (TH1*)gPad->GetPrimitive("delta_adc"); 

	TF1* ff = new TF1("ff", "[0]-expo(1)",520,800);
	ff->SetParameter(0 , 60);

	TF1* f_linear = new TF1("f_linear", "[0]+[1]*x", 725,1200);
	...
	t1->Draw( cor_str + ":adc >> low_corr(900, 400, 1300, 60, 10, 70) ", "adc < 800 && adc > 400 && delta_t > 10 && delta_t < 70 && mycut", "colz");
...
//	dt_adc_low_cor -> DrawCopy("colz");
	...
	t1->Draw("delta_t:adc >> high_corr(900, 400, 1300, 60, 10, 70)", "adc < 1300 && adc >= 800 && delta_t > 10 && delta_t < 70 && mycut", "colz");
...
//	t1->Draw("t0:adc >> time_corr(900, 400, 1300, 120, 120, 240)", "adc < 1300 && adc >= 500", "colz");
	...
	cor_str_low.Form( "((log(%f - delta_t) -%f)/%f)", y_offset, expo_off, expo_muti);
